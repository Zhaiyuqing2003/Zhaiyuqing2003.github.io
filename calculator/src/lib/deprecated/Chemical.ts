...
class ChemicalExpression{
    ...
    static parse(value: string){
        ...
        value = ChemicalExpression.__clarifyMultiplierAndSubscript(value)
    ...
    static __clarifyMultiplierAndSubscript(value: string){
        let pureExpressionPattern = ChemicalExpression.__getPureExpressionPattern()
        let withBracketsValue = value.replace()
    }
    ...
    static __clarifyMultiplier(value: string){
        let integerPart = ChemicalExpression.__getUnsignedIntPattern()
        let withBracketValue = value.replace(ChemicalExpression.__getPureExpressionPattern(), (expression: string) => {
            let execResult = integerPart.exec(expression)
            if (execResult !== null){
                console.log(execResult[0])
                return `${execResult[0]}(${expression.substring(execResult.index + execResult[0].length)})`
            } else {
                return `(${expression})`
            }
        })
        ...
        return withBracketValue.replace(/\(/g, (expression: string, offset: number) => {
            if (offset !== 0 && /\d/.test(withBracketValue[offset - 1])){
                return `*${expression}`
    ...
    static __getStrictUnsignedIntPattern(){
        return /^[0-9]*[1-9]+[0-9]*$/
    }
    ...
    static __parseMolarMass<T>(value: string, parseValueFunction: ParseFunction<T>, parseNumberFunction ?: ParseFunction<T>, parseVariableFunction ?: ParseFunction<T>): T{
        ...
        let multiplierPattern = ChemicalExpression.__getStrictUnsignedIntPattern()
