class Expression{
    ...
    static parse(val: string): (string | OperatorType | KeywordType)[]{
        ...
        let operandPattern = Expression.__getOperandPattern();
        ...
        while (execResult !== null){
            if (operandPattern.test(execResult[0])){
                ...
                while (true){
                    if (expression === "="){
                        // check result, the last element of result could only be a proper variable
                        if (result.length === 0){
                            Expression.__throwNoIdentifierError()
                        } else {
                            // two case
                            // only works for variable type
                            if (!variablePattern.test(result[result.length - 1])){
                    ...
                    if (stack.isEmpty()){
                        if (expression === ")"){
                            // no left bracket to surround it.
                            Expression.__throwMissingLeftBracketError()
                        }
                    ...
                    if (expression === ")"){
                        ...
                        if (operator === undefined){
                            // no value remain, no left bracket to surround it.
                            Expression.__throwMissingLeftBracketError()
                        }
        ...
        while (!stack.isEmpty()){
            ...
            if (operator[operator.length - 1] !== "("){
                ...
                // happens when missing right bracket
                Expression.__throwMissingRightBracketError()
            }
        }
        return result
    }
    ...
    static __getOperandPattern(): RegExp{
        return new RegExp(`^${Expression.__getNumberPatternString()}$|^${Expression.__getVariablePatternString()}$`)
    }
    ...
    static __keyWords: KeywordType[] = [
        "add", "subtract", "multiply", "divide", "exponential", "negate", "assign", "register", "log", "log10", "lg"
    ]
    ...
    static __throwNoIdentifierError(): never{
        throw new SyntaxError("Illegal Expression: no identifier as variable for assignment operator!")
    }
    static __throwMissingLeftBracketError(): never{
        throw new SyntaxError("Illegal Expression: Missing Left Bracket!")
    }
    static __throwMissingRightBracketError(): never{
        throw new SyntaxError("Illegal Expression: Missing Right Bracket!")
    }
    ...
    static __throwCannotInstantiateError(): never{
        throw new SyntaxError(`Cannot Instantiate ${ Expression.name }!`)
    }
}
class Stack<T>{
    value : Array<T>
...
type KeywordType = ("add" | "subtract" | "multiply" | "divide" | "exponential" | "negate" | "assign" | "register" | 
    "log" | "log10" | "lg" | "ln" | "exp" | "sin" | "cos" | "tan")
